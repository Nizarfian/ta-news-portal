...
$berita_utama = mysqli_query($conn, "SELECT * FROM `berita` JOIN kategori ON berita.id_kategori=kategori.id_kategori ORDER BY tgl_rilis DESC LIMIT 3;");
$berita_utama_ids = [];
          ...
          <div class="col-md-12 col-lg-12 p-b-20">
            ...
            <div class="flex-wr-s-c m-rl--7 p-t-10">
              <?php for ($i = 1; $i <= $total_pages; $i++) : ?>
                <a href="index.php?page=<?= $i ?>&search=<?= isset($_GET['search']) ? $_GET['search'] : ''; ?>" class="flex-c-c pagi-item hov-btn1 trans-03 m-all-7 <?= $i == $current_page ? 'pagi-active' : ''; ?>">
                  <?= $i ?>
                </a>
              <?php endfor; ?>
            </div>
